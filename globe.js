...
DAT.Globe = function(container, colorFn) {
  ...
  function init() {
    ...
    if(zoomCount!=0)
    {
      if(centrex!=mouseOnDoubleClick.x)
      {
        var centre_PtSlope = (centrey - mouseOnDoubleClick.y)/(centrex - mouseOnDoubleClick.x);
        var centre_PtDist = Math.sqrt(Math.pow((centrey - mouseOnDoubleClick.y),2)  + Math.pow((centrex - mouseOnDoubleClick.x),2));

        if(centre_PtSlope>0) 
        {
          if(mouseOnDoubleClick.y >= centrey) 
          {
            centrex = centrex + centre_PtDist * Math.cos(Math.atan(centre_PtSlope));
            centrey = centrey + centre_PtDist * Math.sin(Math.atan(centre_PtSlope));
          }
      ...
      else
      {
        if(mouseOnDoubleClick.y >= centrey) 
        {
          centrey = centrey - 2*centre_PtDist;
        }
    ...
    */
    ...
    container.addEventListener('click', onSingleClick, false);
...
//        size = data[i + 2];
  ...
  function onSingleClick(event)
  {
    ...
    //console.log("IMM AFTER CLICKING: " + conRot.toString());

    window.setTimeout(function()
      {
      //console.log("5 SECONDS LATER: "); 
      conRot = 0.005;
      //console.log(conRot.toString()); 
      singleClick = false; }, 5000);
